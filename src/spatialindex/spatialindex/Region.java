...
public class Region implements IShape
{
	...
	{
		if (pLow.length != pHigh.length) throw new IllegalArgumentException();

		m_pLow = new double[pLow.length];
		System.arraycopy(pLow, 0, m_pLow, 0, pLow.length);  
		//(Object src, int srcPos, Object dest, int destPos, int length)
		//(原数组，原数组中的起始位置，目标数组，目标数组中的起始位置，要复制的数组元素的数量)
	...
	{
		if (low.m_pCoords.length != high.m_pCoords.length) throw new IllegalArgumentException();
	...
	{
		if (m_pLow.length != r.m_pLow.length) throw new IllegalArgumentException();
	...
	{
		if (m_pLow.length != p.m_pCoords.length) throw new IllegalArgumentException();
	...
	public boolean touches(final Point p)
	{
		...
		for (int cIndex = 0; cIndex < m_pLow.length; cIndex++)   //维度的递增，x，y，z···
		{
	...
	public double[] getCoordinate(){
		System.out.println("Error: In a region!");
		return null;
	}
	//---------------------------------------------------------------------------------------------------
	...
	{
		if (pToModify.m_pLow.length != pConst.m_pLow.length) throw new IllegalArgumentException();
