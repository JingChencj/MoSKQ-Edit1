	...
	RTreeLoad(String[] args)
	{
		try
		{
			if (args.length != 4)
			{
				...
				//输入：input_file, tree_file, capacity, query_type
				//(query_type)intersection：交叉索引； 10NN：最近邻索引
				System.exit(-1);
			}
			...
			ISpatialIndex tree = new RTree(ps2, file);     //新建RTree
            //--------------------------------------------------------------------------------------------
			...
			while (line != null)
			{
				...
				op = new Integer(st.nextToken()).intValue();   //操作指标，0,1,2
				id = new Integer(st.nextToken()).intValue();   //第二个数，编号，非重复
				...
				keyword = new String(st.nextToken());
				...
				else if (op == 2)
				{
					...
					else if (args[3].equals("10NN"))
					{
						...
						tree.nearestNeighborQuery(10, p, keyword, vis);
