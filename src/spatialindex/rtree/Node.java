...
abstract class Node implements INode
{
	...
	protected int m_capacity = -1;
		// Specifies the node capacity.定义节点容量

	protected Region m_nodeMBR = null;
		// The minimum bounding region enclosing all data contained in the node. 这个节点的最小矩形区域

	protected byte[][] m_pData = null;//用于存储keyword
		// The data stored in the node.

	protected Region[] m_pMBR = null;
		// The corresponding data MBRs.相应的数据矩阵

	protected int[] m_pIdentifier = null;
		// The corresponding data identifiers.相应的数据指针
	...
	protected HashMap<String,ArrayList<Integer>> m_invertedList = new HashMap<>();
		//String是指keyword，ArrayList中存储节点编号
	...
	//
	// IEntry interface  最后叶节点当中的Object or Entry
	//
	...
	protected Node(RTree pTree, int id, int level, int capacity)
	{
		...
		m_invertedList = new HashMap<String,ArrayList<Integer>>();
	}

	protected void insertEntry(byte[] pData, Region mbr, int id) throws IllegalStateException//添加entry肯定是添加到子节点当中！
	{
...
//	protected void insertEntry2(int[] bucket, Region mbr, int id) throws IllegalStateException//添加entry肯定是添加到子节点当中！
//	{
//		if (m_children >= m_capacity) throw new IllegalStateException("m_children >= m_nodeCapacity");
//
//		m_pDataLength[m_children] = (bucket != null) ? bucket.length : 0;
//		m_pData[m_children] = bucket;
//		m_pMBR[m_children] = mbr;
//		m_pIdentifier[m_children] = id;
//
//		m_totalDataLength += m_pDataLength[m_children];
...
//	}
	...
	{
		if (m_children < m_capacity)//如果孩子节点的数目比节点的容量数量要少，那么
		{
			boolean adjusted = false;
			boolean b = m_nodeMBR.contains(mbr);//判断m_nodeMBR中是否包含现在即将插入的mbr

			insertEntry(pData, mbr, id);
			m_pTree.writeNode(this);

			if (! b && ! pathBuffer.empty())//如果m_nodeMBR中不包含即将插入的mbr并且pathBuffer不为空
			{
			...
			/*
			 * 如果孩子节点的数目等于或者大于每个节点所能容纳的节点的数目，并且pathBuffer不为空并且R树的m_treeVariant为3并且在等级m_level上超载表（overflowTable）为空，那么
			 */
		{
		...
		{
			Node[] nodes = split(pData, mbr, id);//这里的id是否就是还未分裂的节点的？？
			Node n = nodes[0];
			Node nn = nodes[1];
...
//	protected boolean insertData2(int[] bucket, Region mbr, int id, Stack pathBuffer, boolean[] overflowTable)
//	{
//		if (m_children < m_capacity)//如果孩子节点的数目比节点的容量数量要少，那么
//		{
//			boolean adjusted = false;
...
//
//			ArrayList vReinsert = new ArrayList(), vKeep = new ArrayList();
//			reinsertData(bucket, mbr, id, vReinsert, vKeep);
//
//			int lReinsert = vReinsert.size();
//			int lKeep = vKeep.size();
//
...
//		else//除上述两种情况之外的其它情况
//		{
//			Node[] nodes = split(bucket, mbr, id);//这里的id是否就是还未分裂的节点的？？
//			Node n = nodes[0];
//			Node nn = nodes[1];
//
//			if (pathBuffer.empty())
...
//	}
	...
	protected int[] pickSeeds()
	{
		...
		double inefficiency = Double.NEGATIVE_INFINITY;//低效率
		...
		switch (m_pTree.m_treeVariant)
		{
			...
			case SpatialIndex.RtreeVariantRstar:
				for (cDim = 0; cDim < m_pTree.m_dimension; cDim++)//dimension维数
				{
